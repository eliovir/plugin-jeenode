...
class jeenodeReal extends eqReal {
    ...
    public function preSave() {
        if ($this->getLogicalId() == '' || !is_numeric($this->getLogicalId())) {
            throw new Exception(__('Le node ID ne peut etre vide et doit etre un nombre', __FILE__));
        }
    }
    ...
    public function getHttpHeader() {
        if ($this->getType() == 'master') {
            ...
            if (count($jeenodeMaster) != 1) {
                log::add('jeenode', 'error', 'Jeenode id : ' . $this->getConfiguration('masterId') . __(' n\'est pas unique', __FILE__));
                return;
            } else {
...
class jeenode extends eqLogic {
    ...
    public static function event() {
        ...
        if (count($jeenodeReal) == 0) {
            log::add('jeenode', 'error', 'Jeenode id : ' . $nodeId . __(' inconnue', __FILE__));
            return;
        }
        if (count($jeenodeReal) > 1) {
            log::add('jeenode', 'error', 'Jeenode id : ' . $nodeId . __(' n\'est pas unique', __FILE__));
            return;
        }
        ...
        if (is_object($jeenodeReal)) {
            if ($type == 'b') {
                ...
                $message = __('L\'équipement <a href="', __FILE__) . $linkToEquipement . '">' . $jeenodeReal->getName();
                $message .= __('</a> à une batterie faible', __FILE__);
...
class jeenodeCmd extends cmd {
    ...
    public function execute($_options = null) {
        ...
        if ($jeenode->getLogicalId() == 'I2C') {
            ...
            $result = $request->exec();
            if ($result == 'NR') {
                throw new Exception(__('Erreur lors de la recuperation d\'information ', __FILE__) . $jeenode->getName() . ' (' . $jeenodeReal->getName() . __('). Retour jeenode : ', __FILE__) . $result);
            }
            ...
            if ($this->getType() == 'action' && $_options != null && $result != $value) {
                throw new Exception(__('Erreur lors de l\'éxécution de l\'action sur ', __FILE__) . $jeenode->getName() . ' (' . $jeenodeReal->getName() . __('). Retour jeenode : ', __FILE__) . $result);
            }
            return $result;
        }
    }
